function [train_h0_data, train_h0_label, train_h1_data, train_h1_label, test_h0_label, test_h1_label]=svm_adad(index, data_name)
...
[train_h0_data, train_h0_label, test_h0_label] = train_h0(index, X, Y, selection_method, Y_temp,Ytemp);

[train_h0_data, train_h0_label] = energy_normalization(train_h0_data, train_h0_label);

[train_h1_data, train_h1_label, test_h1_label] = train_h1(index, X, Y, selection_method, Y_temp,Ytemp);

[train_h1_data, train_h1_label] = energy_normalization(train_h1_data, train_h1_label);

end

function [train_h0_data, train_h0_label,  test_h0_label] = train_h0(index, X, Y, selection_method, Y_temp,Ytemp)
Y_AD=[];
X_train = double(X);
...
% feature Selection on training data
    ...
    k = numF;     
    n=20;
    train_h0_data1 = X_train(:,ranking(1:k));
    train_h0_label = Y_temp;
    index1=find(train_h0_label==0);
    index2=find(index1==index);
    train_h0_data=train_h0_data1(index1,:);
    for i=1:max(size(index1))
        if Ytemp(index1(i))==1
            Y_AD=[ Y_AD;1];
        else
             Y_AD=[ Y_AD;0];
    ...
    Y_ADtemp=logical(Y_AD);
    Y_AD = nominal(ismember(Y_ADtemp,1)); 
    Xtrain = double(train_h0_data);
    Ytrain = (double(Y_AD)-1)*2-1;                       %  labels:  neg_class: -1, pos_class: +1
    test_h0_label = double(Y_AD(index2));
    rank = spider_wrapper(Xtrain, Ytrain , n, lower('rfe'));
    b=ranking(1:k);
    a=rank(1:n);
    train_h0_label=  Y_ADtemp;
    train_h0_data = train_h0_data1(index1,a);
    train_h0_data(index2,:)=[];
    train_h0_label(index2)=[];
...
function [train_h1_data, train_h1_label, test_h1_label] = train_h1(index, X, Y, selection_method, Y_temp,Ytemp)
...
% feature Selection on training data
    ...
    Ytrain = (double(Y_AD)-1)*2-1;                       %  labels:  neg_class: -1, pos_class: +1
    test_h1_label = double(Y_AD(index2));
    rank = spider_wrapper(Xtrain, Ytrain , n, lower('rfe'));
    b=ranking(1:k);
    a=rank(1:n);
    Y_ADtemp(index2) =  ~Y_ADtemp(index2);
    train_h1_label= Y_ADtemp;
    train_h1_data = train_h0_data1(index1,a);
    train_h1_data(index2,:)=[];
    train_h1_label(index2)=[];
